...
The goal of a recommender system is to present lists that both
\emph{match user preferences} (i.e., align with explicit or inferred user preferences) and
\emph{have reasonable value} (pricing consistent with market prices). In the early stages
of system deployment, due to the cold-start problem (small user base and limited
interaction history), standalone collaborative filtering methods are impractical. A
purely content-based approach, while feasible, ignores the \textbf{rating signals}
already computed by the XGBoost model. The design therefore adopts a
\textbf{combined content-based and rating-based} ranking model that
integrates both information sources into a single rating.
