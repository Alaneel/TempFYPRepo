...
DB_USER = os.getenv('DB_USER', 'alanwang') # Updated to system user
DB_PASS = os.getenv('DB_PASS', '') # Empty password for system auth
...
class Agent(Base):
    ...
    source_id = Column(String(100), nullable=True) # ID from original source if available
    
    # Composite unique constraint to prevent duplicate agents
    # Using mobile + name is a reasonable proxy for uniqueness across platforms
    __table_args__ = (
...
class Listing(Base):
    ...
    description = Column(Text) # from original source 'raw_text' or similar if available
    url = Column(String(500), unique=True) # Unique URL to prevent duplicate listings
...
def ingest_data():
    ...
    # Path relative to this script: ../data/aggregated_listings.csv
    ...
    for index, row in df.iterrows():
        ...
        if agent_mobile is not None:
             agent_mobile = str(agent_mobile).replace('.0', '') # Clean float string conversions
        
        agent_cea = row.get('cea')
        agent_desc = row.get('agent_description')
        agent_url = row.get('agent_url')
        agent_source_id = row.get('agent_id') # string ID from source
        
        # Check if agent exists
        if agent_mobile:
            agent = session.query(Agent).filter_by(name=agent_name, mobile=agent_mobile).first()
        else:
            # Fallback for agents without mobile number (can be risky if names collide)
            # Maybe just check by name + cea?
            criteria = {'name': agent_name}
            if agent_cea:
                criteria['cea'] = agent_cea
        ...
        # 2. Insert Listing
        # Check if listing exists by URL (strongest unique identifier across scrapes)
        listing_url = row.get('url')
        existing_listing = session.query(Listing).filter_by(url=listing_url).first()
        
        if existing_listing:
            # Update ALL fields to reflect normalization changes
            ...
            existing_listing.beds = row.get('beds')  # Use get_int? No, 'beds' in CSV is string "3 Beds". 
            # Wait, get_int is defined below in the else block.
            # We need to reuse get_int logic or move it up.
    ...
    session.commit()
    print(f"Ingestion Complete. New Agents: {agents_processed}, Listings: {listings_processed}")
