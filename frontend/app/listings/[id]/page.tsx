          ...
          <div className="lg:col-span-2 space-y-8">
             <div>
                <div className="flex flex-wrap gap-2 mb-3">
                   ...
                   <Badge variant="outline">{listing.buy_rent === 'Rent' ? 'For Rent' : 'For Sale'}</Badge>
                   {listing.tenure && <Badge variant="secondary">{listing.tenure}</Badge>}
                </div>
                <h1 className="text-3xl md:text-4xl font-bold mb-2">{listing.title}</h1>
                <div className="flex items-center text-muted-foreground text-lg">
                   <MapPin className="h-5 w-5 mr-2" />
                   {listing.address || listing.street_name || "Singapore"} {listing.district && `(D${listing.district})`}
                </div>
             </div>

             <div className="grid grid-cols-3 md:grid-cols-4 gap-4 py-6 border-y">
                <div className="flex flex-col">
                   <span className="text-muted-foreground text-sm flex items-center gap-1"><BedDouble className="h-4 w-4"/> Beds</span>
                   <span className="text-xl font-semibold">{listing.beds || 0}</span>
                </div>
                <div className="flex flex-col">
                   <span className="text-muted-foreground text-sm flex items-center gap-1"><Bath className="h-4 w-4"/> Baths</span>
                   <span className="text-xl font-semibold">{listing.baths || 0}</span>
                </div>
                <div className="flex flex-col">
                   <span className="text-muted-foreground text-sm flex items-center gap-1"><Ruler className="h-4 w-4"/> Sqft</span>
                   <span className="text-xl font-semibold">{listing.sqft ? listing.sqft.toLocaleString() : "-"}</span>
                </div>
                <div className="flex flex-col">
                   <span className="text-muted-foreground text-sm flex items-center gap-1"><Building className="h-4 w-4"/> Built</span>
                   <span className="text-xl font-semibold">{listing.built_year ? listing.built_year.replace(/\D/g, '') : "N/A"}</span> 
                </div>
             </div>
                    ...
                    <div className="flex flex-wrap gap-2">
                        {(() => {
                            try {
                                ...
                                return <p className="text-muted-foreground">{listing.facilities_json}</p>;
                            } catch (e) {
                                return <p className="text-muted-foreground">{listing.facilities_json}</p>;
                            }
                        })()}
                 ...
                 <div className="grid grid-cols-1 md:grid-cols-2 gap-y-4 gap-x-8">
                     ...
                     {(listing.condo?.top_date || listing.built_year) && (
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">{listing.condo?.top_date || listing.built_year}</div>
                                 <div className="text-xs text-muted-foreground">TOP Year</div>
                             </div>
                         </div>
                     )}
                     ...
                     {(listing.tenure || listing.condo?.tenure) && (
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">{listing.tenure || listing.condo?.tenure}</div>
                     ...
                     <div className="flex items-start gap-3">
                         <Calendar className="h-5 w-5 text-muted-foreground mt-0.5" />
                         <div>
                             <div className="text-sm font-medium">{new Date(listing.created_at).toLocaleDateString('en-GB', { day: 'numeric', month: 'short', year: 'numeric' })}</div>
                     ...
                     {listing.sqft && (
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">{listing.sqft.toLocaleString()} sqft</div>
                         ...
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">
                                     S$ {listing.psf.toLocaleString('en-SG', { minimumFractionDigits: 2, maximumFractionDigits: 2 })} psf
                                 </div>
                     ...
                     {(listing.condo?.developer_name || listing.developer_name) && (
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">{listing.condo?.developer_name || listing.developer_name}</div>
                     ...
                     {listing.condo?.total_units && (
                         <div className="flex items-start gap-3">
                             ...
                             <div>
                                 <div className="text-sm font-medium">{listing.condo.total_units} units</div>
                   ...
                   <div className="space-y-4">
                      ...
                      <Button className="w-full" size="lg" asChild>
                         <a href={`tel:${listing.agent?.mobile || ""}`}>
                           ...
                           {listing.agent?.mobile ? "Contact Agent" : "No Contact Info"}
                         </a>
                      </Button>
                      <Button variant="outline" className="w-full" onClick={() => {
                         if (listing.agent?.mobile) {
                            window.open(`https://wa.me/65${listing.agent.mobile.replace(/\D/g, '')}`, '_blank');
                         } else {
